...
Главная идея заключается в том, чтобы цена обязательно была выгодной. В этом и заключается цель обучения - заставить систему расчитывать наиболее удачную цену. Обучение происходит при помощи Economic.yaml файла:
...
```
были загружены результаты обучения экономической системы:
...
Попробуем провести в файле Economic.yaml некоторые изменения и повторить операции обучения нашей экономической системы. Проанализируем результаты этих операций, сравнив их друг с другом.
...
Сравнение будем проводить с графиками первых изменений (первое изменение - красный цвет, четвертое изменение - тёмно-зелёный):
...
Потери уменьшаются - изменения в лучшую сторону для policy loss, значение value loss стремится к нулю.
...
Помимо уменьшения значения gamma были решено уменьшить значение strength. Так как такие изменения делают данную операцию более похожей на нулевую, изначальную итерацию, сравнение будет проводиться именно с ней (нулевая итерация - синий цвет, пятая - оранжевый):
...
+ Графики всех итераций из категории Policy (Economic6 = пятая итерация, так как Economic5 из-за незначительных результатов была убрана из итогового отчета):
...
- **Policy loss** - величина изменения политики - элемента, определяющего действия, с проходом определенного количества времени. В основном график этого значения должен стремиться вниз, так как это показывает, что политика с каждым разом принимает решения все лучше и лучше.
- **Value loss** - средняя потеря функции значения. Этот график моделирует то, насколько хорошо агент прогнозирует значение своего следующего состояния. До стабилизации вознаграждения он должен стремиться к нулю, уменьшаться, после же - расти. Несмотря на то, что во время всех проведённых тренировок это значение уменьшалось, я считаю такой результат довольно удачным для небольшого объема тренировочных данных.
